...
public class RagVectorStoreService {
    ...
    public List<RagProductHit> search(String question, List<RagProductDocument> documents, int topK) {
        ...
        for (RagProductVector vector : vectors) {
            ...
            double cosine = cosineSimilarity(queryVector, candidate);
            double lexical = lexicalOverlapScore(queryTokens, tokenize(doc.document()));
            double score = (cosine * 0.82d) + (lexical * 0.18d);
    ...
    private double lexicalOverlapScore(Set<String> left, Set<String> right) {
        ...
        return (double) intersection / (double) Math.max(1, left.size());
    }
