...
public class RagChatService {
	...
	public RagChatService(
			...
			@Value("${app.rag.retrieval.min-score:0.20}") double minScore,
	...
	public RagChatResponse ask(String question, List<RagChatTurn> history, String focusProductName) {
		...
		if (normalizedQuestion.isEmpty()) {
			return new RagChatResponse("Ban hay nhap cau hoi cu the ve gia, size, loai san hoac thuong hieu.", List.of());
		}
		...
		String searchQuery = buildSearchQuery(normalizedQuestion, history, focusProductName);
		List<RagVectorStoreService.RagProductHit> hits = ragVectorStoreService.search(searchQuery, docs, topK).stream()
				.filter(hit -> hit.score() >= minScore)
				.toList();
		if (hits.isEmpty()) {
			return new RagChatResponse(
					"Minh chua tim thay ket qua du phu hop. Ban thu mo ta ro hon (gia, loai san, size, thuong hieu) de minh loc chinh xac hon.",
					List.of()
			);
		}
		...
		String answer = llmAnswerService.generateAnswer(normalizedQuestion, context, suggestions, normalizedHistory, normalizeFocusProductName(focusProductName));
